...
- It drives the cluster with **`kubectl`** (the `shell` tool) against the kube-apiserver —
  **locally**, no SSH, no install. The `nqrust-hypervisor` skill is the playbook.
...
- RantaiClaw with an LLM provider (`rantaiclaw onboard`) and the `nqrust-hypervisor` skill
  deployed (`./install.sh`).
...
See [`skill/nqrust-hypervisor/SKILL.md`](../skill/nqrust-hypervisor/SKILL.md) for the full command
