...
# (claw-ui cloned, deps installed, NQRust brand applied) + a bun runtime + the offline
# installer. Run this on a CONNECTED machine of the SAME arch/OS (Linux x86_64) as the
# airgapped target — it fetches claw-ui + node_modules here so the target needs no network.
#
...
TAG="${2:-v0.0.0}"
ARCH="$(uname -m)"
NAME="nqrust-airgapped-${TAG}-${ARCH}-linux"
...
# 1. base bundle (binary + skills + nqvm + web-ui.sh + theme + setup.sh + docs)
say "→ building base bundle…"
bash "$HERE/build-bundle.sh" "$RC_BIN" "$TAG" >/dev/null
rm -rf "$STAGE"; mkdir -p "$OUT"
tar xzf "$OUT/nqrust-infra-ai-${TAG}-${ARCH}-linux.tar.gz" -C "$OUT"
mv "$OUT/nqrust-infra-ai-${TAG}-${ARCH}-linux" "$STAGE"

# 2. prebuild the web console: fetch claw-ui + deps, then apply the NQRust brand.
...
rm -rf "$_rc_home"
bash "$STAGE/scripts/apply-theme.sh" "$STAGE/web-ui" >/dev/null || true
